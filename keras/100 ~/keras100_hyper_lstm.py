...
x_train = x_train.reshape(x_train.shape[0], 98, 8).astype('float') / 255
x_test = x_test.reshape(x_test.shape[0], 98, 8) / 255
...
def build_model(drop = 0.5, optimizer = 'adam'):
    inputs = Input(shape = (98, 8), name = 'input')
    x = LSTM(512, activation = 'relu', name = 'h1')(inputs)
    x = Dropout(drop)(x)
